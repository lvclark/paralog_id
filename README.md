...
estimate $H_{ind}/H_E$ for each locus.  `src/hindhe.R` is a wrapper for the
Rcpp function to estimate $H_{ind}/H_E$, and will run directly on a `RADdata`
object.  `src/using_hindhe.Rmd` is a tutorial for using this function.
...
`pipeline/process_sam_multi.py` processes a SAM file generated by Bowtie2 with
...
`pipeline/isoloci_fun.py` has functions for estimating the Gini-Simpson index
and Hind/He, swapping haplotypes, and performing tabu search using
Python.  `scripts/isoloci_fun_test.py` tests these functions.

`pipeline/process_isoloci.py` runs through Hind/He estimation and tabu search
using the files output by `pipeline/process_sam_multi.py`
